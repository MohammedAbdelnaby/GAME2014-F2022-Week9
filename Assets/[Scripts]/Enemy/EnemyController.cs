...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        isGroundedAhead = Physics2D.Linecast(groundPoint.position, aheadPoint.position, groundLayerMask);
        isGrounded = Physics2D.OverlapCircle(groundPoint.position, groundRadius, groundLayerMask);

        if (isGrounded && isGroundedAhead)
        {
            Move();
        }

        if (!isGroundedAhead || isObstacleAhead)
        {
            Flip();
        }
    }
